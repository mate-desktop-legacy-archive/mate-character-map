...
MucharmapUnicodeVersion mucharmap_get_unicode_version(gunichar uc)
{
	gint min = 0;
	gint mid;
...
gint mucharmap_get_unihan_count()
{
	return 0;
}
...
/* does a binary search; also caches most recent, since it will often be
 * called in succession on the same character */
static const NamesList* get_nameslist(gunichar uc)
{
	static gunichar most_recent_searched;
...
_mucharmap_unicode_has_nameslist_entry (gunichar uc)
{
...
gunichar *
mucharmap_get_nameslist_exes (gunichar uc)
{
  const NamesList *nl;
  gunichar *exes;
  gunichar i, count;
...
const gchar** mucharmap_get_nameslist_equals(gunichar uc)
{
	const NamesList* nl;
	const gchar** equals;
	gunichar i, count;
...
/* returns newly allocated null-terminated array of gchar* */
/* the items are const, but the array should be freed by the caller */
const gchar** mucharmap_get_nameslist_stars(gunichar uc)
{
	const NamesList* nl;
	const gchar** stars;
	gunichar i, count;
...
/* returns newly allocated null-terminated array of gchar* */
/* the items are const, but the array should be freed by the caller */
const gchar** mucharmap_get_nameslist_pounds(gunichar uc)
{
	const NamesList* nl;
	const gchar** pounds;
	gunichar i, count;
...
/* returns newly allocated null-terminated array of gchar* */
/* the items are const, but the array should be freed by the caller */
const gchar** mucharmap_get_nameslist_colons(gunichar uc)
{
	const NamesList* nl;
	const gchar** colons;
	gunichar i, count;
...
gboolean mucharmap_unichar_validate(gunichar ch)
{
	return g_unichar_validate(ch);
}

/**
 * mucharmap_unichar_to_printable_utf8
 ...
 **/
gint
mucharmap_unichar_to_printable_utf8 (gunichar uc, gchar *outbuf)
{
  ...
  if (! mucharmap_unichar_validate (uc) || (! mucharmap_unichar_isgraph (uc)
      && mucharmap_unichar_type (uc) != G_UNICODE_PRIVATE_USE))
    return 0;
  else if (mucharmap_unichar_type (uc) == G_UNICODE_COMBINING_MARK
      || mucharmap_unichar_type (uc) == G_UNICODE_ENCLOSING_MARK
      || mucharmap_unichar_type (uc) == G_UNICODE_NON_SPACING_MARK)
    {
      gint x;
 ...
 **/
GUnicodeType
mucharmap_unichar_type (gunichar uc)
{
  gint min = 0;
  gint mid;
 ...
 **/
gboolean
mucharmap_unichar_isdefined (gunichar uc)
{
  return mucharmap_unichar_type (uc) != G_UNICODE_UNASSIGNED;
}

/**
 * mucharmap_unichar_isgraph:
 * @uc: a Unicode character
 *
 ...
 **/
gboolean
mucharmap_unichar_isgraph (gunichar uc)
{
  GUnicodeType t = mucharmap_unichar_type (uc);
