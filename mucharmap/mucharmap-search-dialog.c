...
//{
	...
	typedef struct _MucharmapSearchState MucharmapSearchState;

	struct _MucharmapSearchState
	{
	...
	struct _MucharmapSearchDialogPrivate
	{
	  MucharmapWindow       *guw;
	  GtkWidget             *entry;
	...
	static void mucharmap_search_dialog_class_init (MucharmapSearchDialogClass *klass);
	static void mucharmap_search_dialog_init       (MucharmapSearchDialog *dialog);
	...
	static gboolean
	matches (MucharmapSearchDialog *search_dialog,
		     gunichar               wc,
		     ...
		     const gboolean         annotations)
	{
	  ...
	  haystack = mucharmap_get_unicode_data_name (wc);
	  if (haystack)
		{
	  ...
	  if (annotations)
	...
	#if ENABLE_UNIHAN
		  haystack = mucharmap_get_unicode_kDefinition (wc);
		  if (haystack)
		{
		  priv->search_state->strings_checked++;
		...
		return TRUE;
		  ...
		  haystack_arr = mucharmap_get_nameslist_equals (wc);
		  if (haystack_arr)
		{
		...
		return TRUE;
		  ...
		  haystack_arr = mucharmap_get_nameslist_stars (wc);
		  if (haystack_arr)
		{
		  for (i = 0; haystack_arr[i] != NULL; i++)
			{
		...
		return TRUE;
		  ...
		  haystack_arr = mucharmap_get_nameslist_colons (wc);
		  if (haystack_arr)
		{
		  for (i = 0; haystack_arr[i] != NULL; i++)
			{
		...
		return TRUE;
		  ...
		  haystack_arr = mucharmap_get_nameslist_pounds (wc);
		  if (haystack_arr)
		{
		  for (i = 0; haystack_arr[i] != NULL; i++)
			{
	...
	static gint
	check_for_explicit_codepoint (const MucharmapCodepointList *list,
		                          const gchar                  *string)
	{
	  const gchar *nptr;
	  gchar *endptr;
	  ...
	  if (nptr != string)
		{
		  ...
		  if (endptr != nptr)
		    {
		      gint index = mucharmap_codepoint_list_get_index ((MucharmapCodepointList *) list, wc);
		      if (index != -1)
		        return index;
		    }
		}
	...
	static gboolean
	quick_checks_before (MucharmapSearchState *search_state)
	{
	...
	static gboolean
	quick_checks_after (MucharmapSearchState *search_state)
	{
	  ...
	  if (!search_state->whole_word)
		if (search_state->search_string_nfd[0] < 0x20 || search_state->search_string_nfd[0] > 0x7e)
		  {
		gint index = mucharmap_codepoint_list_get_index (search_state->list, g_utf8_get_char (search_state->search_string_nfd));
		if (index != -1)
		  {
	  ...
	  do
		{
		  ...
		  wc = mucharmap_codepoint_list_get_char (priv->search_state->list, priv->search_state->curr_index);

		  if (!mucharmap_unichar_validate (wc) || !mucharmap_unichar_isdefined (wc))
		    continue;
	...
	mucharmap_search_state_get_found_char (MucharmapSearchState *search_state)
	{
	  if (search_state->found_index > 0)
		return mucharmap_codepoint_list_get_char (search_state->list, search_state->found_index);
	  else
		return (gunichar)(-1);
	}
	...
	mucharmap_search_state_free (MucharmapSearchState *search_state)
	{
	  ...
	  g_slice_free (MucharmapSearchState, search_state);
	}

	/**
	 * mucharmap_search_state_new:
	 * @list: a #MucharmapCodepointList to be searched
	 ...
	 *
	 * Initializes a #MucharmapSearchState to search for the next character in
		                        ...
		                        gboolean                      annotations)
	{
	  ...
	  search_state->list_num_chars = mucharmap_codepoint_list_get_last_index (search_state->list) + 1;
	  ...
	  if (search_state->search_string_nfd_len == 1)
		search_state->search_index_nfd = mucharmap_codepoint_list_get_index (search_state->list, g_utf8_get_char (search_state->search_string_nfd));
	  else
	  ...
	  if (search_state->search_string_nfc_len == 1)
		search_state->search_index_nfc = mucharmap_codepoint_list_get_index (search_state->list, g_utf8_get_char (search_state->search_string_nfc));
	  else
	...
	search_completed (MucharmapSearchDialog *search_dialog)
	{
	  ...
	  g_signal_emit (search_dialog, mucharmap_search_dialog_signals[SEARCH_FINISH], 0, found_char);
		                                  ...
		                                  MucharmapDirection     direction)
	{
	  ...
	  if (priv->search_state == NULL
		  ...
		  || priv->search_state->annotations != gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (priv->annotations_option)) )
		{
		  ...
		  start_char = mucharmap_charmap_get_active_character (priv->guw->charmap);
		  start_index = mucharmap_codepoint_list_get_index (list, start_char);
		  priv->search_state = mucharmap_search_state_new (list, gtk_entry_get_text (GTK_ENTRY (priv->entry)),
		...
		{
		  start_char = mucharmap_charmap_get_active_character (priv->guw->charmap);
		  priv->search_state->start_index = mucharmap_codepoint_list_get_index (list, start_char);
	  ...
	  g_signal_emit (search_dialog, mucharmap_search_dialog_signals[SEARCH_START], 0);
	...
	mucharmap_search_dialog_finalize (GObject *object)
	{
	  ...
	  G_OBJECT_CLASS (mucharmap_search_dialog_parent_class)->finalize (object);
	}

	static void
	mucharmap_search_dialog_class_init (MucharmapSearchDialogClass *clazz)
	{
	  g_type_class_add_private (clazz, sizeof (MucharmapSearchDialogPrivate));

	  G_OBJECT_CLASS (clazz)->finalize = mucharmap_search_dialog_finalize;
	...
	mucharmap_search_dialog_get_completed (MucharmapSearchDialog *search_dialog)
	{
	  ...
	  if (priv->search_state == NULL || !priv->search_state->searching)
		...
		{
	#if ENABLE_UNIHAN
		  gdouble total = mucharmap_get_unicode_data_name_count () + mucharmap_get_unihan_count ();
	#else
		  gdouble total = mucharmap_get_unicode_data_name_count ();
	#endif
