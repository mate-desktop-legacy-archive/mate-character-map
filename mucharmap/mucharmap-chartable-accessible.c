...
//{
	typedef struct
	{
	  ...
	  GPtrArray *cells;
	  AtkObject *focus_obj;
	} MucharmapChartableAccessiblePrivate;

	#define GET_PRIVATE(inst) (G_TYPE_INSTANCE_GET_PRIVATE ((inst), mucharmap_chartable_accessible_get_type (), MucharmapChartableAccessiblePrivate))

	static gpointer mucharmap_chartable_accessible_parent_class;

	static void
	set_cell_visibility (MucharmapChartable  *chartable,
		                 MucharmapChartableCellAccessible  *cell,
		                 gboolean emit_signal)
	{
	  MucharmapChartablePrivate *chartable_priv = chartable->priv;
	  mucharmap_chartable_cell_accessible_add_state (cell, ATK_STATE_VISIBLE, emit_signal);
	  ...
	  if (cell->index >= chartable_priv->page_first_cell &&
		...
		{
		  mucharmap_chartable_cell_accessible_add_state (cell, ATK_STATE_SHOWING, emit_signal);
		}
	  else 
		{
		  mucharmap_chartable_cell_accessible_remove_state (cell, ATK_STATE_SHOWING, emit_signal);
		}
	}

	static void
	cell_destroyed (MucharmapChartableCellAccessible *cell)
	{
	  MucharmapChartableAccessiblePrivate *priv;
	  AtkObject *parent;
	  GPtrArray *cells;
	  guint n_cells, n;
	  ...
	  for (n = 0; n < n_cells; ++n)
		{
		  MucharmapChartableCellAccessible *another_cell = g_ptr_array_index (cells, n);
	...
	static AtkObject*
	mucharmap_chartable_accessible_ref_child (AtkObject *obj,
		                                      int index)
	{
	  MucharmapChartableAccessible *table;
	  MucharmapChartableAccessiblePrivate *priv;
	  GtkWidget *widget;
	...
	static AtkObject* 
	mucharmap_chartable_accessible_ref_at (AtkTable *table,
		                         gint    row,
		                         gint    column)
	{
	  MucharmapChartable *chartable;
	  ...
	  child = mucharmap_chartable_accessible_ref_child (ATK_OBJECT (table), index);

	  return child;
	}

	static AtkObject*
	mucharmap_chartable_accessible_ref_accessible_at_point (AtkComponent *component,
		                                          gint x, 
		                                          gint y,
		                                          AtkCoordType coord_type)
	{
	  ...
	  for (col = 0; col < chartable_priv->cols; col++)
		{
		  if (x_pos < _mucharmap_chartable_x_offset (chartable, col))
		    {
		      col--;
		      break;
		    }
	  ...
	  for (row = 0; row < chartable_priv->rows; row++)
		{
		  if (y_pos < _mucharmap_chartable_y_offset (chartable, row))
		    {
		      row--;
		      break;
		    }
	  ...
	  return mucharmap_chartable_accessible_ref_at (ATK_TABLE (component), row, col);
	}

	static void
	mucharmap_chartable_accessible_component_interface_init (AtkComponentIface *iface)
	{
	  g_return_if_fail (iface != NULL);

	  iface->ref_accessible_at_point = mucharmap_chartable_accessible_ref_accessible_at_point;
	}

	static AtkStateSet*
	mucharmap_chartable_accessible_ref_state_set (AtkObject *obj)
	{
	...
	static gint
	mucharmap_chartable_accessible_get_n_children (AtkObject *obj)
	{
	  GtkWidget *widget;
	...
	static void
	mucharmap_chartable_accessible_init (MucharmapChartableAccessible *accessible)
	{
	...
	mucharmap_chartable_accessible_finalize (GObject *obj)
	{
	  ...
	  G_OBJECT_CLASS (mucharmap_chartable_accessible_parent_class)->finalize (obj);
	}

	static void
	mucharmap_chartable_accessible_update_all_cells (AtkObject *obj)
	{
	  MucharmapChartableAccessible *accessible;
	  MucharmapChartableAccessiblePrivate *priv;
	  GtkWidget *widget;
	...
	adjustment_changed (GtkAdjustment *adj,
		                AtkObject     *obj)
	{
	  mucharmap_chartable_accessible_update_all_cells (obj);
	}
	...
	size_allocated (GtkWidget     *widget,
		            ...
		            gpointer      data)
	{
	  mucharmap_chartable_accessible_update_all_cells (ATK_OBJECT (data));
	}

	static AtkObject* /* reference */
	find_object (MucharmapChartable   *chartable,
		         gunichar  uc,
		         AtkObject *obj)
	{
	  MucharmapChartablePrivate *chartable_priv = chartable->priv;
	  ...
	  column = _mucharmap_chartable_cell_column (chartable, uc);

	  return atk_table_ref_at (ATK_TABLE (obj), row, column);
	}

	static void
	sync_active_char (MucharmapChartable  *chartable,
		              GParamSpec *pspec,
		              MucharmapChartableAccessible *accessible)
	{
	  MucharmapChartableAccessiblePrivate *priv = GET_PRIVATE (accessible);
	  ...
	  uc = mucharmap_chartable_get_active_character (chartable);
	...
	static void
	mucharmap_chartable_accessible_set_scroll_adjustments (MucharmapChartable *chartable,
	...
	static void
	mucharmap_chartable_accessible_initialize (AtkObject *obj,
		                                       gpointer  data)
	{
	  GtkWidget *widget;
	  ...
	  g_signal_connect_after (chartable, "set-scroll-adjustments",
		                      G_CALLBACK (mucharmap_chartable_accessible_set_scroll_adjustments),
		                      obj);
	...
	mucharmap_chartable_accessible_connect_widget_destroyed (GtkAccessible *accessible)
	{
	  ...
	  GTK_ACCESSIBLE_CLASS (mucharmap_chartable_accessible_parent_class)->connect_widget_destroyed (accessible);
	}

	static void
	mucharmap_chartable_accessible_class_init (MucharmapChartableAccessibleClass *klass)
	{
	  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
	  AtkObjectClass *atk_object_class = ATK_OBJECT_CLASS (klass);
	  GtkAccessibleClass *accessible_class = GTK_ACCESSIBLE_CLASS (klass);

	  mucharmap_chartable_accessible_parent_class = g_type_class_peek_parent (klass);

	  gobject_class->finalize = mucharmap_chartable_accessible_finalize;

	  accessible_class->connect_widget_destroyed = mucharmap_chartable_accessible_connect_widget_destroyed;

	  atk_object_class->get_n_children = mucharmap_chartable_accessible_get_n_children;
	  atk_object_class->ref_child = mucharmap_chartable_accessible_ref_child;
	  atk_object_class->ref_state_set = mucharmap_chartable_accessible_ref_state_set;
	  atk_object_class->initialize = mucharmap_chartable_accessible_initialize;

	  g_type_class_add_private (gobject_class, sizeof (MucharmapChartableAccessiblePrivate));
	}

	static gint
	mucharmap_chartable_accessible_get_n_columns (AtkTable *table)
	{
	  GtkWidget *widget;
	  MucharmapChartable *chartable;
	  MucharmapChartablePrivate *chartable_priv;
	...
	static gint
	mucharmap_chartable_accessible_get_column_extent_at (AtkTable       *table,
		                                       gint           row,
		                                       gint           column)
	{
	  GtkWidget *widget;
	...
	static gint
	mucharmap_chartable_accessible_get_n_rows (AtkTable *table)
	{
	  GtkWidget *widget;
	  MucharmapChartable *chartable;
	  MucharmapChartablePrivate *chartable_priv;
	...
	static gint
	mucharmap_chartable_accessible_get_row_extent_at (AtkTable *table,
		                                    gint    row,
		                                    gint    column)
	{
	  GtkWidget *widget;
	...
	static gint
	mucharmap_chartable_accessible_get_index_at (AtkTable *table,
		                               gint     row,
		                               gint     column)
	{
	  GtkWidget *widget;
	...
	static gint
	mucharmap_chartable_accessible_get_column_at_index (AtkTable *table,
		                                      gint     index)
	{
	  GtkWidget *widget;
	  MucharmapChartable *chartable;
	...
	static gint
	mucharmap_chartable_accessible_get_row_at_index (AtkTable *table,
		                                             gint     index)
	{
	  GtkWidget *widget;
	  MucharmapChartable *chartable;
	...
	static void
	mucharmap_chartable_accessible_table_interface_init (AtkTableIface *iface)
	{
	  iface->ref_at = mucharmap_chartable_accessible_ref_at;
	  iface->get_n_columns = mucharmap_chartable_accessible_get_n_columns;
	  iface->get_column_extent_at = mucharmap_chartable_accessible_get_column_extent_at;
	  iface->get_n_rows = mucharmap_chartable_accessible_get_n_rows;
	  iface->get_row_extent_at = mucharmap_chartable_accessible_get_row_extent_at;
	  iface->get_index_at = mucharmap_chartable_accessible_get_index_at;
	  iface->get_column_at_index = mucharmap_chartable_accessible_get_column_at_index;
	  iface->get_row_at_index = mucharmap_chartable_accessible_get_row_at_index;
	}
	...
	ATK_DEFINE_TYPE_WITH_CODE (MucharmapChartableAccessible,
		                       ...
		                       G_IMPLEMENT_INTERFACE (ATK_TYPE_COMPONENT,
		                                              mucharmap_chartable_accessible_component_interface_init))

	#else

	GType
	mucharmap_chartable_accessible_get_type (void)
	{
	  ...
	  if (g_once_init_enter (&type__volatile))
		{
		  GTypeInfo typeinfo =
		  {
		    ...
		    (GClassInitFunc) mucharmap_chartable_accessible_class_init, /* class init */
		    (GClassFinalizeFunc) NULL, /* class finalize */
		    NULL, /* class data */
		    0 /* filled in by the code below */,
		    0, /* nb preallocs */
		    (GInstanceInitFunc) mucharmap_chartable_accessible_init, /* instance init */
		    NULL /* value table */
		  };
		  const GInterfaceInfo atk_table_info =
		  {
		      (GInterfaceInitFunc) mucharmap_chartable_accessible_table_interface_init,
		      (GInterfaceFinalizeFunc) NULL,
		      NULL
		  };
		  const GInterfaceInfo atk_component_info =
		  {
		      (GInterfaceInitFunc) mucharmap_chartable_accessible_component_interface_init,
		      (GInterfaceFinalizeFunc) NULL,
		      NULL
		  };
	...
	AtkObject* 
	mucharmap_chartable_accessible_new (MucharmapChartable *chartable)
	{
	  GObject *object;
	  AtkObject *accessible;

	  object = g_object_new (mucharmap_chartable_accessible_get_type (), NULL);
